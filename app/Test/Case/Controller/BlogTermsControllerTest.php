<?php
/**
 * BlogTermsController Test Case
 *
 * @author   Jun Nishikawa <topaz2@m0n0m0n0.com>
 * @link     http://www.netcommons.org NetCommons Project
 * @license  http://www.netcommons.org/license.txt NetCommons License
 */

App::uses('BlogTermsController', 'Controller');

/**
 * Summary for BlogTermsController Test Case
 */
class BlogTermsControllerTest extends ControllerTestCase {

/**
 * Fixtures
 *
 * @var array
 */
	public $fixtures = array(
		'app.blog_term',
		'app.content',
		'app.module',
		'app.archive',
		'plugin.users.user_group',
		'plugin.users.user_group_link',
		'plugin.users.user',
		'app.authority',
		'app.authority_lang',
		'app.module_link',
		'app.module_system_link',
		'app.page_user_link',
		'app.community_invitation',
		'app.page_sum_view',
		'app.page',
		'app.block',
		'app.blog_style',
		'app.whatsnew_select_room',
		'app.whatsnew_select_user',
		'app.whatsnew_style',
		'app.page_column',
		'app.page_layout',
		'app.page_meta',
		'app.page_style',
		'app.page_theme',
		'app.passport',
		'app.upload',
		'app.upload_link',
		'plugin.users.user_item_link',
		'plugin.users.user_item',
		'plugin.users.user_item_authority_link',
		'plugin.users.user_item_lang',
		'app.config_lang',
		'app.config',
		'app.blog_comment',
		'app.blog_post',
		'app.blog_term_link',
		'app.blog'
	);

/**
 * testIndex method
 *
 * @return void
 */
	public function testIndex() {
	}

/**
 * testView method
 *
 * @return void
 */
	public function testView() {
	}

/**
 * testAdd method
 *
 * @return void
 */
	public function testAdd() {
	}

/**
 * testEdit method
 *
 * @return void
 */
	public function testEdit() {
	}

/**
 * testDelete method
 *
 * @return void
 */
	public function testDelete() {
	}

}
